...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Super Star!', tokenId2, {from: user2});
...
it('lets a user transfer a star', async() => {
    ...
    assert.equal(user2, await instance.ownerOf.call(tokenId));
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let instance = await StarNotary.deployed();
    let user = accounts[5];
    await instance.createStar('Amazing Star!', tokenId, {from: user});
    let name = await instance.lookUptokenIdToStarInfo(tokenId, {from: user})
    assert.equal('Amazing Star!', name);
